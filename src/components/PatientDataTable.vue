<template>
    <div>
        <div class="card">
            <DataTable ref="dt" :value="patients" v-model:selection="selectedPatients" dataKey="id" 
                paginator :filters="filters" :loading="loading" lazy
                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport"
                currentPageReportTemplate="Showing {first} to {last} of {totalRecords} patients" responsiveLayout="scroll"
                :rows="10" :totalRecords="pagination.totalRecords"
                @page="loadPatients($event.page+1)" @sort="onSort"
            >
                <template #header>
                    <div class="table-header flex flex-column md:flex-row md:justify-content-between">
						...
						<div class="flex-end">
                            <Button label="Delete" icon="pi pi-trash" class="p-button-danger" @click="confirmDeleteSelected" :disabled="!selectedPatients || !selectedPatients.length" />
                ...
                <Column :exportable="false" style="min-width:8rem">
                    <template #body="slotProps">
                        <Button icon="pi pi-pencil" class="p-button-rounded mr-2" @click="startEditingPatient(slotProps.data)" />
                        <Button icon="pi pi-trash" class="p-button-danger p-button-rounded" @click="confirmDeletePatient(slotProps.data)" />
                    </template>
                </Column>
                <template #paginatorstart>
                    <Button type="button" icon="pi pi-refresh" class="p-button-text" @click="loadPatients(pagination.currentPage)"/>
                </template>
        ...
        <Dialog v-model:visible="patientDialog" :style="{width: '450px'}" header="Patient Details" :modal="true" class="p-fluid">
            <img src="https://www.primefaces.org/wp-content/uploads/2020/05/placeholder.png" :alt="patient.image" class="patient-image" v-if="patient.image" />
            <div class="field">
                <label for="firstName">First name</label>
                <InputText id="firstName" v-model.trim="patient.firstName" required="true" autofocus :class="{'p-invalid': submitted && !patient.firstName}" />
                <small class="p-error" v-if="submitted && !patient.firstName">First name is required.</small>
            </div>
            <div class="field">
                <label for="lastName">Last name</label>
                <InputText id="lastName" v-model.trim="patient.lastName" required="true" autofocus :class="{'p-invalid': submitted && !patient.lastName}" />
                <small class="p-error" v-if="submitted && !patient.lastName">Last name is required.</small>
            </div>
            <div class="field">
                <label for="email">Email</label>
                <InputText id="email" v-model.trim="patient.email" required="true" autofocus :class="{'p-invalid': submitted && !patient.email}" />
                <small class="p-error" v-if="submitted && !patient.email">Email is required.</small>
            </div>
            <div v-if="!editingPatient" class="field">
                <label for="password">Password</label>
                <InputText id="password" v-model.trim="patient.password" required="true" autofocus :class="{'p-invalid': submitted && !patient.password}" />
                <small class="p-error" v-if="submitted && !patient.password">Password is required.</small>
            </div>

            <template #footer>
                <Button label="Cancel" icon="pi pi-times" class="p-button-text" @click="hideDialog"/>
                <Button label="Save" icon="pi pi-check" class="p-button-text" @click="() => editingPatient ? saveEditedPatient() : saveNewPatient()" />
            </template>
        </Dialog>
        ...
        <Dialog v-model:visible="deletePatientDialog" :style="{width: '450px'}" header="Confirm" :modal="true">
            <div class="confirmation-content">
                ...
                <span v-if="patient">Are you sure you want to delete <b>{{patient.firstName}} {{patient.lastName}}</b>?</span>
            </div>
            <template #footer>
...
const loadPatients = (page = 1) => {
    ...
    getPatients(page)
        ...
        .catch(err => {
            console.error(err);
            toast.add({severity:'error', summary: err?.response?.statusText || 'Error', detail: err?.response?.data?.msg || 'Could not fetch patients', life: 3000});
        })
...
const saveEditedPatient = () => {
    ...
    if(!patient.value.firstName || !patient.value.lastName || !patient.value.email) return;
    editPatient(patient.value.id, patient.value.firstName, patient.value.lastName, patient.value.email)
        .then(() => {
        ...
        .catch(err => {
            console.error(err);
           toast.add({severity:'error', summary: err?.response?.statusText || 'Error', detail: err?.response?.data?.msg || 'Could not edit patient', life: 3000});
        })
        .finally(() => {
            patientDialog.value = false;
            editingPatient.value = false;
...
const startEditingPatient = (doct) => {
    patient.value = {...doct};
    editingPatient.value = true;
    patientDialog.value = true;
};
const confirmDeletePatient = (doct) => {
    patient.value = {...doct};
...
const deletePatientCallback = () => {
    deletePatient(patient.value.id)
        .then(() => {
            toast.add({severity:'success', summary: 'Success', detail: `Patient ${patient.value.firstName} ${patient.value.lastName} removed`, life: 3000});
            loadPatients();
        })
        .catch(err => {
            console.error(err);
            toast.add({severity:'error', summary: err?.response?.statusText || 'Error', detail: err?.response?.data?.msg || 'Could not delete patient', life: 3000});
        })
        .finally(() => {
...
const onSearch = () => {
    patients.value = patientsBackup.value.filter(
